...
class Log {
	...
	/// Logs network messages on console with prefix [📍]
	///
	/// - Parameters:
	///   - object: Object or message to be logged
	///   - funcName: Name of the function from where the logging is done
	class func n ( _ object: Any, filename: String = #file, line: Int = #line, column: Int = #column, funcName: String = #function) {
		if isLoggingEnabled {
			print("\(Date().toString()) \(LogEvent.n.rawValue) \(funcName) -> \(object)")
		}
	}
